...
// NodeEncoder will take an id (hash, or key for leaf servicers), the depth of the node,
...
// Size returns the number of leaf servicers in the tree.
...
// IterateRange makes a callback for all servicers with key between start and end non-inclusive.
...
// IterateRangeInclusive makes a callback for all servicers with key between start and end inclusive.
// If either are nil, then it is open on that side (nil, nil is the same as Iterate). The keys and
// values must not be modified, since they may point to data stored within IAVL.
...
// nodeSize is like Size, but includes inner servicers too.
