...
func (k *Keeper) InitMemStore(ctx sdk.Ctx) {
	...
	if !k.IsInitialized(noGasCtx) {
		...
		iterator, _ := sdk.KVStorePrefixIterator(prefixStore, nil)
...
func (k Keeper) GetOwners(ctx sdk.Context, index uint64) (types.CapabilityOwners, bool) {
	...
	ownerBytes, _ := prefixStore.Get(indexKey)
	if ownerBytes == nil {
...
func (sk ScopedKeeper) GetCapabilityName(ctx sdk.Context, cap *types.Capability) string {
	...
	memStore := ctx.KVStore(sk.memKey)
	a, _ := memStore.Get(types.FwdCapabilityKey(sk.module, cap))
	return string(a)
}
