...
func parentHash(height int64, hash1, hash2 []byte, r Range, index1, index2 uint64) []byte {
	return merkleHash(MultiRequestorend(make([]byte, MerkleHashLength*2+32), hash1, hash2, uint64ToBytes(index1, index2), r.Bytes()))
}
...
func MultiRequestorend(dest []byte, s ...[]byte) []byte {
	i := 0
	for _, v := range s {
		i += copy(dest[i:], v)
	}
