...
func (a AAT) Validate() error {
	...
	// check the requestor signature of the aat
...
func (a AAT) Bytes() []byte {
	...
	b, err := json.Marshal(AAT{
		RequestorSignature: "",
		RequestorPublicKey: a.RequestorPublicKey,
...
func (a AAT) ValidateMessage() error {
	// check for valid requestor public key
	if len(a.RequestorPublicKey) == 0 {
		return MissingRequestorPublicKeyError
	}
	if err := PubKeyVerification(a.RequestorPublicKey); err != nil {
		return err
	}
