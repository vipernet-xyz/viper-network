...
type Proof interface {
	...
	Validate(requestorSupportedBlockchains []string, sessionNodeCount int, sessionBlockHeight int64) sdk.Error // validate the object
...
func (rp RelayProof) Validate(requestorSupportedBlockchains []string, sessionNodeCount int, sessionBlockHeight int64) sdk.Error {
	...
	c1 := false
	for _, chain := range requestorSupportedBlockchains {
...
func (rp RelayProof) SessionHeader() SessionHeader {
	return SessionHeader{
		RequestorPubKey:    rp.Token.RequestorPublicKey,
		Chain:              rp.Blockchain,
		GeoZone:            rp.GeoZone,
...
func (c ChallengeProofInvalidData) Validate(requestorSupportedBlockchains []string, sessionNodeCount int, sessionBlockHeight int64) sdk.Error {
	...
	// check for identical requestor public keys
	if majResponse.Proof.Token.RequestorPublicKey != majResponse2.Proof.Token.RequestorPublicKey ||
		majResponse2.Proof.Token.RequestorPublicKey != c.MinorityResponse.Proof.Token.RequestorPublicKey ||
		majResponse.Proof.Token.RequestorPublicKey != c.MinorityResponse.Proof.Token.RequestorPublicKey {
		return NewMismatchedRequestorPubKeyError(ModuleName)
	}
