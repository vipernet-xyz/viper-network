...
func TestRelayProof_ValidateLocal(t *testing.T) {
	...
	requestorSignature, er := requestorPrivateKey.Sign(validProof.Token.Hash())
	if er != nil {
		t.Fatalf(er.Error())
	}
	validProof.Token.RequestorSignature = hex.EncodeToString(requestorSignature)
	...
	invalidProofInvalidAAT.Token.RequestorSignature = hex.EncodeToString(clientSignature) // wrong signature
	...
	invalidProofClientSignature.Signature = hex.EncodeToString(requestorSignature) // wrong signature
	tests := []struct {
		name             string
		proof            Proof
		maxRelays        int64
...
func TestRelayProof_SessionHeader(t *testing.T) {
	...
	validProof.Token.RequestorSignature = hex.EncodeToString(requestorSignature)
	sh := SessionHeader{
		RequestorPubKey:    validProof.Token.RequestorPublicKey,
...
func TestChallengeProofInvalidData_ValidateLocal(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := SessionHeader{
				RequestorPubKey:    tt.proof.MinorityResponse.Proof.Token.RequestorPublicKey,
				Chain:              tt.proof.MinorityResponse.Proof.Blockchain,
				SessionBlockHeight: tt.proof.MinorityResponse.Proof.SessionBlockHeight,
			}
			if err := tt.proof.ValidateLocal(h, tt.maxRelays, tt.supportedBlockchains, 5, tt.sessionNodes, tt.reporterAddress, GlobalEvidenceCache); (err != nil) != tt.hasError {
				fmt.Println(tt.name)
				fmt.Println(err)
				t.Fatalf(err.Error())
			}
...
func NewValidChallengeProof(t *testing.T) (challenge ChallengeProofInvalidData, ser1 crypto.PrivateKey, ser2 crypto.PrivateKey, ser3 crypto.PrivateKey, requestor crypto.PrivateKey, cli crypto.PrivateKey, repor crypto.PrivateKey) {
	requestorPrivateKey := GetRandomPrivateKey()
	...
	requestorSignature, er = requestorPrivateKey.Sign(validProof2.Token.Hash())
	if er != nil {
		t.Fatalf(er.Error())
	}
	validProof2.Token.RequestorSignature = hex.EncodeToString(requestorSignature)
	...
	requestorSignature, er = requestorPrivateKey.Sign(validProof3.Token.Hash())
	if er != nil {
		t.Fatalf(er.Error())
	}
	validProof3.Token.RequestorSignature = hex.EncodeToString(requestorSignature)
	...
	}, servicerPrivKey1, servicerPrivKey2, servicerPrivKey3, requestorPrivateKey, clientPrivateKey, reporterPrivKey
}
