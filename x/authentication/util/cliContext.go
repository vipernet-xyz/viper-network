...
type CLIContext struct { // TODO consider module passing clicontext instead of servicer and keybase
	Codec         *codec.Codec
...
func NewCLIContext(servicer rpcclient.Client, fromAddress sdk.Address, passphrase string) CLIContext {
	return CLIContext{
		Client:        servicer,
...
// BroadcastTxCommit broadcasts transaction bytes to a Tendermint servicer and
// waits for a commit. An error is only returned if there is no RPC servicer
...
func (ctx CLIContext) BroadcastTxCommit(txBytes []byte) (sdk.TxResponse, error) {
	servicer, err := ctx.GetNode()
	if err != nil {
		return sdk.TxResponse{}, err
	}

	res, err := servicer.BroadcastTxCommit(txBytes)
	if err != nil {
...
// BroadcastTxSync broadcasts transaction bytes to a Tendermint servicer
...
func (ctx CLIContext) BroadcastTxSync(txBytes []byte) (sdk.TxResponse, error) {
	...
	res, err := servicer.BroadcastTxSync(txBytes)
	return sdk.NewResponseFormatBroadcastTx(res), err
}

// BroadcastTxAsync broadcasts transaction bytes to a Tendermint servicer
...
func (ctx CLIContext) BroadcastTxAsync(txBytes []byte) (sdk.TxResponse, error) {
	...
	res, err := servicer.BroadcastTxAsync(txBytes)
	return sdk.NewResponseFormatBroadcastTx(res), err
}
...
// ---------------------------------------------------------------------------------------------------------------------
// Query performs a query to a Tendermint servicer with the provided path.
...
// QueryWithData performs a query to a Tendermint servicer with the provided path
...
// QueryStore performs a query to a Tendermint servicer with the provided key and
...
// query performs a query to a Tendermint servicer with the provided store name
...
func (ctx CLIContext) query(path string, key bytes.HexBytes) (res []byte, height int64, err error) {
	...
	result, err := servicer.ABCIQueryWithOptions(path, key, opts)
	if err != nil {
		return res, height, err
	}
...
// queryStore performs a query to a Tendermint servicer with the provided a store
...
func (ctx CLIContext) GetAccountWithHeight(addr sdk.Address) (exported.Account, int64, error) {
	bs, err := authentication.ModuleCdc.MarshalJSON(types.NewQueryAccountParams(addr))
	if err != nil {
		return nil, 0, err
	}
	res, height, err := ctx.QueryWithData(fmt.Sprintf("custom/%s/%s", authentication.QuerierRoute, authentication.QueryAccount), bs)
	if err != nil {
		return nil, height, err
	}
	var account exported.Account
	if err := authentication.ModuleCdc.UnmarshalJSON(res, &account); err != nil {
		return nil, height, err
	}
