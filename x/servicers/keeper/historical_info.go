...
func (k Keeper) SetHistoricalInfo(ctx sdk.Ctx, height int64, hi *types.HistoricalInfo) {
	store := ctx.KVStore(k.storeKey)
	key := types.GetHistoricalInfoKey(height)
...
func (k Keeper) DeleteHistoricalInfo(ctx sdk.Ctx, height int64) {
	store := ctx.KVStore(k.storeKey)
	key := types.GetHistoricalInfoKey(height)
...
func (k Keeper) IterateHistoricalInfo(ctx sdk.Ctx, cb func(types.HistoricalInfo) bool) {
	store := ctx.KVStore(k.storeKey)

	iterator, _ := sdk.KVStorePrefixIterator(store, types.HistoricalInfoKey)
...
func (k Keeper) GetAllHistoricalInfo(ctx sdk.Ctx) []types.HistoricalInfo {
...
func (k Keeper) TrackHistoricalInfo(ctx sdk.Ctx) {
	entryNum := 1000
