...
func TestPool_CoinsFromUnstakedToStaked(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			platform: types.Platform{Address: platformAddress},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case true:
				...
				err := keeper.coinsFromUnstakedToStaked(context, tt.platform, tt.amount)
				assert.NotNil(t, err)
			default:
				addMintedCoinsToModule(t, context, &keeper, types.StakedPoolName)
				sendFromModuleToAccount(t, context, &keeper, types.StakedPoolName, tt.platform.Address, sdk.NewInt(100000000000))
...
func TestPool_CoinsFromStakedToUnstaked(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			platform: types.Platform{Address: platformAddress, StakedTokens: sdk.NewInt(10)},
			amount:   sdk.NewInt(110),
			panics:   false,
		},
		{
			name:     "errors if negative ammount",
			platform: types.Platform{Address: platformAddress, StakedTokens: sdk.NewInt(-1)},
			amount:   sdk.NewInt(-1),
			want:     "negative coin amount: -1",
			panics:   true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case true:
				...
				if strings.Contains(tt.name, "setup") {
					addMintedCoinsToModule(t, context, &keeper, types.StakedPoolName)
					sendFromModuleToAccount(t, context, &keeper, types.StakedPoolName, tt.platform.Address, sdk.NewInt(100))
				}
				_ = keeper.coinsFromStakedToUnstaked(context, tt.platform)
			default:
			}
		})
	}
...
func TestPool_BurnStakedTokens(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case true:
				addMintedCoinsToModule(t, context, &keeper, types.StakedPoolName)
				sendFromModuleToAccount(t, context, &keeper, types.StakedPoolName, tt.platform.Address, supplySize)
				_ = keeper.coinsFromUnstakedToStaked(context, tt.platform, tt.amount)
...
func TestPool_StakedRatio(t *testing.T) {
	...
	tests := []struct {
		...
		{"return 0 if stake supply is lower than 0", sdk.ZeroDec(), platformAddress},
		{"return supply", sdk.NewDec(1), platformAddress},
	}
