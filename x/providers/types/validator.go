...
// a reduction of 10^6 from validator tokens is platformlied
...
func (v Validator) RemoveStakedTokens(tokens sdk.BigInt) (Validator, error) {
	if tokens.IsNegative() {
		return Validator{}, fmt.Errorf("should not hplatformen: trying to remove negative tokens: %s from valdiator %s", tokens.String(), v.Address)
	}
	if v.StakedTokens.LT(tokens) {
		return Validator{}, fmt.Errorf("should not hplatformen: only have %v tokens, trying to remove %v", v.StakedTokens, tokens)
	}
...
func (v Validator) AddStakedTokens(tokens sdk.BigInt) (Validator, error) {
	if tokens.IsNegative() {
		return Validator{}, fmt.Errorf("should not hplatformen: trying to add negative tokens: %s from valdiator %s", tokens.String(), v.Address)
	}
