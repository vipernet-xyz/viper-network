...
func init() {
	...
	startCmd.Flags().BoolVar(&keybase, "keybase", true, "run with keybase, if disabled allows you to stake for the current validator only. providing a keybase is still neccesary for staking for requestors & sending transactions")
	...
	startCmd.Flags().BoolVar(&forceSetValidatorsLean, "forceSetValidators", false, "reads your lean_viper_user_key_file (lean_nodes_keys.json) and updates your last signed state/validator files before starting your node")
...
func start(cmd *cobra.Command, args []string) {
	...
	go rpc.StartRPC(app.GlobalConfig.ViperConfig.RPCPort, app.GlobalConfig.ViperConfig.RPCTimeout, simulateRelay, profileApp, allBlockTxs, app.GlobalConfig.ViperConfig.ChainsHotReload, app.GlobalConfig.ViperConfig.GeoZonesHotReload)
