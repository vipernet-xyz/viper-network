...
func TestRPC_QueryTX(t *testing.T) {
	...
	assert.Nil(t, err)
	tx, err = servicers.Send(memCodec(), memCLI, kb, cb.GetAddress(), cb.GetAddress(), "test", types.NewInt(100), true)
	assert.Nil(t, err)
...
func TestRPC_QueryServicers(t *testing.T) {
	codec.UpgradeHeight = 7000
	_, _, cleanup := NewInMemoryTendermintNode(t, oneValTwoNodeGenesisState())
	_, stopCli, evtChan := subscribeTo(t, tmTypes.EventNewBlock)
	...
	q := newQueryRequest("servicers", newBody(params))
	rec := httptest.NewRecorder()
	Servicers(rec, q, httprouter.Params{})
	body := rec.Body.String()
	address := cb.GetAddress().String()
	assert.True(t, strings.Contains(body, address))
