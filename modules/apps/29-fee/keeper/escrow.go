...
func (k Keeper) DistributePacketFeesOnAcknowledgement(ctx sdk.Ctx, forwardRelayer string, reverseRelayer sdk.Address, packetFees []types.PacketFee, packetID channeltypes.PacketId) {
...
func (k Keeper) DistributePacketFeesOnTimeout(ctx sdk.Ctx, timeoutRelayer sdk.Address, packetFees []types.PacketFee, packetID channeltypes.PacketId) {
	// cache context before trying to distribute fees
	// if the escrow account has insufficient balance then we want to avoid partially distributing fees
	cacheCtx, writeFn := ctx.CacheContext()
...
func (k Keeper) distributeFee(ctx sdk.Context, receiver, refundAddress sdk.Address, fee sdk.Coins) {
	...
	if err != nil {
		if bytes.Equal(receiver, refundAddress) {
		...
		err := k.bankKeeper.SendCoinsFromModuleToAccount(cacheCtx, types.ModuleName, refundAddress, fee)
		if err != nil {
			k.Logger(ctx).Error("error refunding fee to the original sender", "refund address", refundAddress, "fee", fee)
			return // if sending to the refund address fails, no-op
		}

		emitDistributeFeeEvent(ctx, refundAddress.String(), fee)
	} else {
