...
func (k Keeper) SendTx(ctx sdk.Ctx, _ *capabilitytypes.Capability, connectionID, portID string, icaPacketData icatypes.InterchainAccountPacketData, timeoutTimestamp uint64) (uint64, error) {
	return k.sendTx(ctx, connectionID, portID, icaPacketData, timeoutTimestamp)
}

func (k Keeper) sendTx(ctx sdk.Ctx, connectionID, portID string, icaPacketData icatypes.InterchainAccountPacketData, timeoutTimestamp uint64) (uint64, error) {
...
func (k Keeper) OnTimeoutPacket(ctx sdk.Ctx, packet channeltypes.Packet) error {
	return nil
}
