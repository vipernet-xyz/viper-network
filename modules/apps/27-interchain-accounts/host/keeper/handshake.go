...
func (k Keeper) OnChanOpenTry(
	...
	interchainAddr, found := k.GetInterchainAccountAddress(ctx, metadata.HostConnectionId, counterparty.PortId)
	if found {
		// reopening an interchain account
		k.Logger(ctx).Info("reopening existing interchain account", "address", interchainAddr)
		Address = sdk.MustAddressFromBech32(interchainAddr)
		if _, ok := k.accountKeeper.GetAccount(ctx, Address).(*icatypes.InterchainAccount); !ok {
			return "", errorsmod.Wrapf(icatypes.ErrInvalidAccountReopening, "existing account address %s, does not have interchain account type", Address)
		}

	} else {
		Address, err = k.createInterchainAccount(ctx, metadata.HostConnectionId, counterparty.PortId)
		if err != nil {
			return "", err
		}
		k.Logger(ctx).Info("successfully created new interchain account", "host-connection-id", metadata.HostConnectionId, "port-id", counterparty.PortId, "address", Address)
	}

	metadata.Address = Address.String()
