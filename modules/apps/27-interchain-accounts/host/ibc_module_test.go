	...
	testCases := []struct {
		...
		},
		{
			"success: ICA auth module callback returns error", func() {
				...
				suite.chainB.GetSimApp().ICAAuthModule.IBCApp.OnChanOpenTry = func(ctx sdk.Ctx, order channeltypes.Order, connectionHops []string,
		...
		},
		{
			"success with ICA auth module callback failure", func() {
				suite.chainB.GetSimApp().ICAAuthModule.IBCApp.OnRecvPacket = func(
					ctx sdk.Ctx, packet channeltypes.Packet, relayer sdk.AccAddress,
...
func (suite *InterchainAccountsTestSuite) fundICAWallet(ctx sdk.Ctx, portID string, amount sdk.Coins) {
