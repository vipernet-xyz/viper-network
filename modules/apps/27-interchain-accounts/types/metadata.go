...
func ValidateControllerMetadata(ctx sdk.Ctx, channelKeeper ChannelKeeper, connectionHops []string, metadata Metadata) error {
...
func ValidateHostMetadata(ctx sdk.Ctx, channelKeeper ChannelKeeper, connectionHops []string, metadata Metadata) error {
	if !isSupportedEncoding(metadata.Encoding) {
		return errorsmod.Wrapf(ErrInvalidCodec, "unsupported encoding format %s", metadata.Encoding)
	}
